...
# -------imputation setup -----------
maincond <- read.csv("maincond4_RandUCB.csv")
names <- matrix(NA,nrow = nrow(maincond), ncol = 1)
mylist <- list()

# currently 12*36*1= 432 imputations
...
for(i in 1:nrow(maincond)){
  ...
  p0 = maincond$p0[i]
  p1 = maincond$p1[i]
  reticulate::source_python("imputation4_RandUCB.py")
...
saveRDS(mylist, 'mylist_imputation4_RandUCB_1st.rds')
