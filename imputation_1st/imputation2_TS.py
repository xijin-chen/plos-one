...
current_success_prob = [[[0 for ___ in range(K)] for __ in range(T+1)] for _ in range(N)]
...
for n in range(N):
    ...
    for t in range(1, T+1):
        ...
        for k in range(K):
            indices_value[k] = method(arms_prior[k][0], S[n][t-1][k], 
                         arms_prior[k][1], F[n][t-1][k], T, t-1, K,
                         S=S, F=F,
            ...
            for k in range(K):
                S[n][t][k] = S_imp[n][t][k] + S_obs[n][t][k]
                F[n][t][k] = F_imp[n][t][k] + F_obs[n][t][k] 
